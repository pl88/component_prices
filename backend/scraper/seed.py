...
def seed_mvp_data(session: Session) -> None:
    shop = session.scalar(sqlmodel_select(Shop).where(Shop.name == MVP_SHOP_NAME))
    if shop is None:
        shop = Shop(name=MVP_SHOP_NAME, base_url=MVP_SHOP_URL, created_at=datetime.now(UTC))
        session.add(shop)
        session.flush()
    ...
    for component_name, component_category, product_url in MVP_COMPONENTS:
        component = session.scalar(
            sqlmodel_select(Component).where(Component.name == component_name)
        )
        ...
        mapping = session.scalar(
            sqlmodel_select(ComponentShopURL).where(
